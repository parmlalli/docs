...
<Info>
  SSO is an **Enterprise** feature. Sign-in is **SP-initiated**: users start at Planasonix, then Okta, then return to the ACS. Read [SSO](/settings/sso) for the overall flow before you follow the steps below.
</Info>

## Copy values from Planasonix

Open **Settings → SSO** in Planasonix and copy:
...
<Tip>
  Pilot with a small Okta group first. Always test login from the Planasonix sign-in page (SP-initiated), not from an Okta app tile.
</Tip>
...
Upload Okta’s **metadata XML** or paste **SSO URL**, **issuer**, and **X.509 signing certificate** into Planasonix. Run a test login from an private/incognito window starting at Planasonix.
