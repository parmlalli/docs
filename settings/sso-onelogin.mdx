...
<Info>
  SSO is an **Enterprise** feature. Sign-in is **SP-initiated** (start at Planasonix). See [SSO](/settings/sso).
</Info>

## Planasonix values

From **Settings → SSO**, copy:
...
Paste or upload these in Planasonix **Settings → SSO**.

<Tip>
  Use OneLogin **Mappings** or **Roles** to control which users can authenticate to the Planasonix application. Always test login from the Planasonix sign-in page.
</Tip>

## Certificate rotation

When you renew the OneLogin signing certificate, download fresh metadata and update Planasonix before the previous certificate expires. Run a pilot login from Planasonix after upload.

<Warning>
  If you change the **ACS URL** or **Entity ID** in OneLogin without updating Planasonix (or the reverse), SAML sign-in fails with opaque errors. Treat SP values as read-only from the Planasonix console.
</Warning>
