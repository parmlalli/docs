...
**Delta Lake Destination** writes standards-compliant Delta Lake tables to cloud object storage (S3, GCS, Azure Blob). Each batch produces a Parquet file and an atomic Delta transaction log commit with column statistics.
...
<Info>
  For full configuration details, cloud provider setup, performance benchmarks, and troubleshooting, see the dedicated [Delta Lake destination](/connections/delta-lake) page.
</Info>

## Managed Lakehouse (professional+)

**Managed Lakehouse** writes Parquet data once and commits metadata to **both Apache Iceberg and Delta Lake** simultaneously. Every engine in your stack — whether it reads Iceberg or Delta — can query the same underlying data.
...
<Info>
  For full architecture details, catalog configuration, API reference, and troubleshooting, see the dedicated [Managed Lakehouse](/connections/managed-lakehouse) page.
</Info>
